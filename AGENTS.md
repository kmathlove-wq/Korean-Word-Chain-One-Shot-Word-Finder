...
- 한방단어 모드 전체는 `ONE_SHOT_TIME_BUDGET`(20초) 안에서만 진행한다(Render 앞단이 응답을 약 30초에서 끊는 걸 확인). 넘기면 그때까지 확인한 결과만 반환하고(`ONE_SHOT_TIME_BUDGET_WARNING`) 캐시하지 않는다 — 다음 검색이 데워진 캐시로 더 찾는다.
