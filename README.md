...
검색은 공식 API의 시작 일치 검색을 사용하고, JSON 응답을 우선 처리하며 XML 응답도 안전하게 파싱합니다. 한 묶음은 100개이고 필요한 페이지가 채워질 때까지 최대 10묶음(약 1000개) 범위에서 다음 묶음을 확인하며, 화면에는 24개씩 보여 줍니다. 같은 끝 글자의 조회는 캐시에서 재사용합니다. 메모리 캐시는 서버 재시작 시 초기화되며 다중 프로세스 사이에서는 공유되지 않습니다.
