...
검색 하나는 필터·정렬·기기와 상관없이 15초 안에서만 진행합니다(한방단어 모드, `이어갈 단어가 적은 순`·`한방단어 우선` 정렬, "이어갈 단어 수" 채우기 모두 해당). 그 안에 다 못 끝냈으면 그때까지 확인한 결과만 보여 주고 "시간이 부족해 일부 후보를 확인하지 못했습니다. 같은 글자로 다시 검색하면 이어서 더 찾아냅니다."라는 안내가 함께 나옵니다. 같은 글자로 다시 검색하면 그새 데워진 캐시 덕분에 더 많이 확인됩니다. (한때 8초로 더 짧게 줄여 봤지만, 국립국어원 API가 느린 날엔 한방단어를 거의 못 찾아서 15초로 다시 늘렸습니다. 응답 속도와 "실제로 한방단어를 찾아내는지" 사이의 균형점입니다.)
