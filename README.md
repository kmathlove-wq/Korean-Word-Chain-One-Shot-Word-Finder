...
- 두 단계 로딩: 단어 목록을 먼저 보여주고 "이어갈 단어 수"는 `/api/continuations`로 뒤이어 채움(가나다·짧은·긴 순)
...
검색은 공식 API의 시작 일치 검색을 사용하고, JSON 응답을 우선 처리하며 XML 응답도 안전하게 파싱합니다. 한 묶음은 100개이고 필요한 페이지가 채워질 때까지 최대 10묶음(약 1000개) 범위에서 다음 묶음을 확인하며, 화면에는 24개씩 보여 줍니다. 같은 끝 글자의 조회는 캐시에서 재사용하고, 국립국어원 서버 연결은 공유 세션으로 재사용하며 독립적인 끝 글자 조회는 최대 20개까지 동시에 처리합니다. 메모리 캐시는 서버 재시작 시 초기화되며 다중 프로세스 사이에서는 공유되지 않습니다.

가나다·짧은·긴 순 목록은 두 단계로 나눠 응답합니다. 먼저 단어 목록만 돌려주고(`deferred: true`), 화면이 `/api/continuations`로 끝 글자들의 이어갈 단어 수와 한방단어 표시를 이어서 채웁니다. 그동안 각 카드의 이어갈 단어 수는 "확인 중…"으로 보입니다. `이어갈 단어가 적은 순`·`한방단어 우선` 정렬과 한방단어 모드는 예전처럼 한 번에 계산합니다.
