...
검색 하나는 필터·정렬·기기와 상관없이 15초 안에서만 진행합니다(한방단어 모드, `이어갈 단어가 적은 순`·`한방단어 우선` 정렬, "이어갈 단어 수" 채우기 모두 해당). 그 15초 중 앞부분(약 40%)은 후보를 넓게 모으는 데, 나머지는 실제로 한방단어인지 확인하는 데 씁니다 — 둘이 시간을 나눠 쓰지 않으면 후보가 아주 많은 흔한 글자는 모으다가 시간이 다 끝나 정작 확인을 한 번도 못 해볼 수 있습니다. 그래도 다 못 끝냈으면 그때까지 확인한 결과만 보여 주고 "시간이 부족해 일부 후보를 확인하지 못했습니다. 같은 글자로 다시 검색하면 이어서 더 찾아냅니다."라는 안내가 함께 나옵니다. 같은 글자로 다시 검색하면 그새 데워진 캐시 덕분에 더 많이 확인됩니다.
