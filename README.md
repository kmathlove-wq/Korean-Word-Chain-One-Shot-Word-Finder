...
- 한방단어 모드는 검색어로 시작하는 단어를 최대 3,000개까지 실제로 살펴 판정하므로, 손으로 정해둔 목록 밖의 희귀한 받침으로 끝나는 한방단어도 놓치지 않음
...
검색은 공식 API의 시작 일치 검색을 사용하고, JSON 응답을 우선 처리하며 XML 응답도 안전하게 파싱합니다. 한 묶음은 100개이고 필요한 페이지가 채워질 때까지 최대 10묶음(약 1000개) 범위에서 다음 묶음을 확인하며, 화면에는 24개씩 보여 줍니다. 같은 끝 글자의 조회는 캐시에서 재사용하고, 국립국어원 서버 연결은 공유 세션으로 재사용하며 독립적인 끝 글자 조회는 최대 24개까지 동시에 처리합니다. 운영 서버는 프로세스 1개(스레드 8개)로 실행해 메모리 캐시·연결 풀·예열이 모든 요청에서 공유됩니다. 캐시는 서버 재시작 시 초기화됩니다.
...
한방단어 모드는 검색어로 시작하는 단어를 사전 한 곳에서 최대 3,000개까지 실제로 모아 판정한 뒤, 완결된 목록을 한 번에 돌려줍니다(손으로 정해둔 '희귀 받침 목록'을 추측하지 않으므로 그 목록 밖의 받침으로 끝나는 한방단어도 찾아냅니다). 시작 단어가 아주 많은 흔한 글자는 3,000개 이후에 나오는 극히 희귀한 단어를 여전히 놓칠 수 있습니다.

검색 하나는 필터·정렬·기기와 상관없이 8초 안에서만 진행해 10초 안에 결과를 돌려줍니다(한방단어 모드, `이어갈 단어가 적은 순`·`한방단어 우선` 정렬, "이어갈 단어 수" 채우기 모두 해당). 그 안에 다 못 끝냈으면 그때까지 확인한 결과만 보여 주고 "시간이 부족해 일부 후보를 확인하지 못했습니다. 같은 글자로 다시 검색하면 이어서 더 찾아냅니다."라는 안내가 함께 나옵니다. 같은 글자로 다시 검색하면 그새 데워진 캐시 덕분에 더 많이 확인됩니다.
